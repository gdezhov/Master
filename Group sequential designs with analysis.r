...
ntab2 = c(min(out[,10]), quantile(out[,10],  probs = 0.05), median(out[,10]),
		mean(out[,10]),quantile(out[,10],  probs = 0.95), max(out[,10]))
...
main = "Процент изводи по етапи адаптивен дизайн. ", radius = -1)
...
main = "Процент изводи по етапи групов последователен дизайн. ", radius = -1)
